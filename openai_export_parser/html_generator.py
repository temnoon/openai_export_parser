...
class HTMLGenerator:
    ...
    def generate_conversation_html(
        ...
        # Serialize data for embedding. Escape <, >, & so a value containing
        # "</script>" cannot break out of its <script> block (XSS).
        conv_json = self._embed_json(conversation, indent=2)
        media_files_json = self._embed_json(media_files)
        assets_json = self._embed_json(assets)
        media_mapping_json = self._embed_json(media_mapping)
        asset_pointer_map_json = self._embed_json(asset_pointer_map)
...
<body>
    <div class="container">
        <header>
            <div class="header-content">
                <div class="nav-buttons">
                    ...
                    <button onclick="toggleDarkMode()" class="btn btn-secondary">🌓 Toggle Theme</button>
                </div>
                ...
                <div class="metadata">
                    ...
                    {f'<span>🖼️ Media: {len(media_files)}</span>' if media_files else ''}
                    {f'<span>📄 Assets: {len(assets)}</span>' if assets else ''}
                </div>
            </div>
        </header>
        ...
        <footer>
            <p>Generated by <a href="https://github.com/temnoon/openai_export_parser" target="_blank">OpenAI Export Parser</a></p>
            <p class="conv-id">Conversation ID: {self._escape_html(str(conv_id))}</p>
        </footer>
    </div>

    <!-- Embedded Conversation Data -->
    <script id="conversation-data" type="application/json">{conv_json}</script>
    <script id="media-files-data" type="application/json">{media_files_json}</script>
        <script id="asset-files-data" type="application/json">{assets_json}</script>
        <script id="media-mapping-data" type="application/json">{media_mapping_json}</script>
        <script id="asset-pointer-map-data" type="application/json">{asset_pointer_map_json}</script>
...
{self._get_javascript()}
    ...
    @staticmethod
    def _embed_json(obj, indent=None):
        """Serialize for safe embedding in a <script type="application/json"> tag.

        json.dumps does not escape '<', so a conversation containing the
        literal '</script>' would otherwise close the tag and let the rest
        parse as HTML. Escaping <, >, & as \\uXXXX keeps the JSON valid while
        making tag-breakout impossible.
        """
    ...
    def _get_javascript(self):
        ...
        function renderMessageContent(content) {
            ...
            return DOMPurify.sanitize(html, { ADD_ATTR: ['controls', 'preload', 'loading'] });
        }
        ...
        function renderMessage(nodeId, nodeData) {
            ...
            if (author === 'user') {
                ...
                messageClass = 'tool';
                authorLabel = '🔧 Tool: ' + (message.author.name || 'Unknown');
            } else if (author === 'system') {
                messageClass = 'tool';
                authorLabel = '⚙️ System';
            }
